...
class TGAImg
  ...
  public:
   ...
   int GetWidth();
   int GetHeight();
   unsigned char* GetImg();       // Return a Structs::Pointer to image data
   unsigned char* GetPalette();   // Return a Structs::Pointer to VGA palette
...
int TGAImg::LoadTgaRLEData() // Load RLE compressed image data
  ...
  // Set our Structs::Pointer to the beginning of the image data
  pCur=&pData[iOffset];
...
GLuint C_GLTextureManagerHash::LoadGLTextureTGAFile( std::string path )
{
   ...
   TGAImg img;
   if( !img.Load( (char*) path.c_str() ) )
   {
      //img.GetBPP();
      ::glTexImage2D( GL_TEXTURE_2D, 0, 4, img.GetWidth(), img.GetHeight(), 0, GL_BGRA_EXT, 
                      GL_UNSIGNED_BYTE, img.GetImg() );
