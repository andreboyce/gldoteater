#include "WinFileHandler.h"

int WinFileHandler::SaveFile( std::string & path, char filter[],
...
int WinFileHandler::OpenFile( std::string & path, char filter[],
                                char defexten[], const DWORD& flags,
                                const short& filterindex, char title[] )
{
   short buflen = MAX_PATH+1;
...
void WinFileHandler::SaveMultiSelectFileNames( char* buffer, int FileOffset )
{
   m_FileNames.clear();
...
std::deque< std::string > WinFileHandler::GetMultiSelectFileNames( void )
{
   return m_FileNames;
}

std::string WinFileHandler::GetCurrentDirectory( void )
{
