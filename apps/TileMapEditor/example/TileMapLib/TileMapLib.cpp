...
void C_TileMap::ResizeRow( int index, int width )
{
   ...
   if( m_Tiles.size() >= index )
   {
      if( width == 0 )
      {
         for( int i=m_Tiles[index].size()-1; i>0; --i )
         {
      ...
      {
         int orignalsize = m_Tiles[index].size();
         for( int i = m_Tiles[index].size(); i<width; ++i )
         {
            m_Tiles[index].push_back( S_Tile( 0, 0 ) );
         }
         return;    
      }
      // make smaller
      if( width < m_Tiles[index].size() )
      {
         if( width < 0 )
            return;
         int orignalsize = m_Tiles[index].size();
         for( int i = m_Tiles[index].size()-1; (i>=width) ; --i )
         {
            m_Tiles[index].pop_back();
         }
      }
...
void C_TileMap::ResizeMap( int width, int height )
{
   ...
   {
      int orignalsize = m_Tiles.size();
      for( int i=0; i<height ; ++i )
      {
         if( i >= orignalsize )
         {
            std::vector<S_Tile> row;
            m_Tiles.push_back( row );
            ResizeRow( m_Tiles.size()-1, width );
         }
         else
         {
            ResizeRow( i, width );
...
bool C_TileMap::SaveTileSet( std::string filename )
{
   ...
   {
      tilesetfile << m_TileSet[i] << std::endl;
   }
   tilesetfile.close();
   return true;
}
   ...
   try
   {
      ...
      while( !std::getline( tilemap, line ).eof() )
      {
              ...
              ++beg )
         {
            ...
            row.push_back( S_Tile( graphic, solidity ) );
         }
         m_Tiles.push_back( row );
      }
   }
...
bool C_TileMap::SaveMap( std::string filename )
{
   try
   {
      ...
      std::vector<S_Tile>::iterator tile;
...
void C_TileMap::CopySelectedTiles( void )
{
   ...
   {
      m_TilesSelected.push_back( std::vector<S_Tile>() );
      m_OrignalTilesSelected.push_back( std::vector<S_Tile>() );
      for( int c = m_SelectedTiles.StartX ; c < max_x ; ++c, ++x )
      {
         m_TilesSelected[y].push_back( S_Tile() );
         m_OrignalTilesSelected[y].push_back( S_Tile() );
...
void C_TileMap::GetSelectedTiles( C_Structs::Point m_MousePosition, C_Structs::Point m_RButtonDownStartPoint )
{
