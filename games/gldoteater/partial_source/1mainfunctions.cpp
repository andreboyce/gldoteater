...
Structs::Point MainWindow::GetTile( const Vector2D& position )
{
   ...
   if( !m_TileMap.m_Header.width || !m_TileMap.m_Header.height )
      return tile;
   return Structs::Point( (long)floor( ( position.m_x + m_BoundingRectHalfWidthMinMapTexturedQuadhalfwidth ) * m_InvBoundingRectWidthDivTileMapWidth ),
                             abs( (long)floor( ( position.m_y - m_BoundingRectHalfHeight + m_MapTexturedQuad.halfheight() ) * m_InvBoundingRectHeightDivTileMapHeight ) ) );
}
